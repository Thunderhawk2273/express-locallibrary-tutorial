    ...
    (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            ...
            async.parallel({
                ...
                if (err) { 
                    debug('create_post_input error: ' + err);
                    return next(err); 
                }
            ...
            book.save(function (err) {
                if (err) { 
                    debug('create_post_mongodb_save error: ' + err);
                    return next(err); 
                }
...
exports.book_delete_post = function(req, res) {
    async.parallel({
        ...
        if(results.book_instances.length > 0){
            ...
            Book.findByIdAndDelete(req.body.bookid).exec(function(err, results){
                if(err){
                    debug('delete_post_mongodb error: ' + err);
                    return next(err);}
    ...
    (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            ...
            Book.findByIdAndUpdate(req.params.id, book, {}, function (err,thebook) {
                if (err) {
                    debug('update_post_mongodb error: ' + err);
                    return next(err); }
